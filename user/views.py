...
from .tasks import (
    ...
    send_verification_email_for_pw,
    send_email_with_pw,
)
    ...
    def post(self, request):
        ...
        if social is None:
            ...
            url = (
                ...
                + settings.SOCIAL_AUTH_NAVER_CLIENT_ID
                + "&redirect_uri="
                + BASE_URL
                + "&state="
                + STATE
...
class NaverLoginView(APIView):
    def post(self, request):
        ...
        if access_token_request.status_code != 200:
            return Response(
                {"status": "400", "error": "네이버 로그인 실패. 다시 시도해주세요."},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class MyPageView(APIView):
    ...
    def get(self, request):
        ...
        return Response(
            {"status": "200", "my_data": serializer.data}, status=status.HTTP_200_OK
        )
...
class UserInfoView(APIView):
    ...
    def get(self, request):
        ...
        return Response(
            {"status": "200", "user_info": serializer.data}, status=status.HTTP_200_OK
        )
    ...
    def put(self, request):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response(
                {"status": "200", "success": "회원 정보가 수정되었습니다."},
                status=status.HTTP_200_OK,
            )
        else:
            return Response(
...
class QnaView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            return Response(
                {"status": "201", "success": "건의 작성 완료"}, status=status.HTTP_201_CREATED
            )
        elif "content" in serializer.errors:
            return Response(
                {"status": "400", "error": "내용을 입력해주세요."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        else:
            return Response(
...
class PasswordResetView(APIView):
    ...
    def post(self, request):
        ...
        return Response(
            {"status": "200", "success": "임시 비밀번호 발급을 위한 인증 이메일이 전송되었습니다."},
            status=status.HTTP_200_OK,
        )
...
class SendRandomPassword(APIView):
    ...
    def get(self, request, uidb64, token):
        ...
        return Response(
            {"status": "200", "success": "임시 비밀번호가 이메일로 발송되었습니다."},
            status=status.HTTP_200_OK,
        )
...
def load_payment(request, user):
    ...
    return Response(
        {"status": "200", "order_data": order_data}, status=status.HTTP_200_OK
    )
...
class PaymentPageView(APIView):
    ...
    def post(self, request):
        ...
        # 사용자의 총 결제액 조회
        user_total_payment = PaymentResultModel.objects.filter(
            buyer_email=user.email
        ...
        if total_paid_amount + new_payment_amount >= 20000:
            return Response(
                {
                    "status": "400",
                    "error": "죄송합니다. 결제 제한 초과로 인해 더 이상 결제를 진행할 수 없습니다. 현재까지 결제한 금액은 사용자별로 총 20,000원까지 허용됩니다.",
                },
                status=status.HTTP_400_BAD_REQUEST,
            )
        else:
...
class PaymentResult(APIView):
    ...
    def post(self, request):
        ...
        if request.user.email == request.data.get("rsp", {}).get("buyer_email", ""):
            ...
            return Response(
                {"status": "403", "error": "인증된 사용자의 정보와 일치하지 않습니다."},
...
class UserTicketsView(APIView):
    def get(self, request):
        ...
        if not user.is_authenticated:
            return Response(
                {"error": "인증되지 않은 사용자"}, status=status.HTTP_401_UNAUTHORIZED
            )
