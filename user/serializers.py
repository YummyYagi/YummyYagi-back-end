    ...
    def validate(self, data):
        ...
        if not re.match(pattern, password):
            raise exceptions.ValidationError(
                {
                    "password": "비밀번호는 8자 이상 20자 이하 및 숫자와 특수 문자(#?!@$~%^&*-)를 하나씩 포함시켜야 합니다."
                }
            )
        return data
    ...
    def validate(self, request_data):
        ...
        try:
            ...
            raise exceptions.NotFound(
                {"status": "404", "error": "사용자를 찾을 수 없습니다. 로그인 정보를 확인하세요."}
            )

        if not user.check_password(request_password):
            raise exceptions.AuthenticationFailed(
                {"status": "401", "error": "비밀번호가 일치하지 않습니다."}
            )
        elif user.is_active == False:
            raise exceptions.AuthenticationFailed(
                {"status": "403", "error": "이메일 인증이 필요합니다."}
            )
        else:
...
class MypageSerializer(serializers.ModelSerializer):
    ...
    bookmark_story_list = serializers.SerializerMethodField(
        method_name="get_bookmark_story_list"
    )
    story_timestamps = serializers.SerializerMethodField(
        method_name="get_story_timestamps"
    )
    ...
    def get_bookmark_story_list(self, obj):
        bookmarked_stories = obj.bookmark_stories.filter(hate_count__lte=4).order_by(
            "-created_at"
        )
    ...
    def get_story_timestamps(self, obj):
        stories = (
            Story.objects.all()
            .filter(hate_count__lte=4, timestamps__user=obj)
            .order_by("-timestamps__timestamp")
        )
